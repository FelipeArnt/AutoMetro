...
class AutoMetro: # Implementando a classe Autômetro para lidar melhor com o código.
    ...
    def repetir_acoes(self):
        if self.actions:
            ...
            console.print("Nenhuma ação carregada! Use opção 1 ou 2 primeiro.", style="bold yellow")
    ...
    def run(self):
        while True:
            opt = menu_interativo()
            rodar = self.state_machine.get(opt, lambda: console.print("Opção inválida!", style="bold red"))
            rodar()


def rec():
    console.print("Você tem 5 segundos para ir a tela do software de ensaio.", style="bold yellow")
    time.sleep(5)
    # Registra ações do usuário (cliques do mouse ou pressionamentos de teclas).

    actions = []
    console.rule("[bold blue]Modo de Gravação[/]")
    console.print("Clique onde deseja repetir a ação. Pressione 'Ctrl+C' para parar.", style="italic")
    # Loop while.
    try:
        while True:
            # Tipo de ação.
            ...
            if action_type == 'm':
                ...
                console.print(f"Posição do clique registrada: [bold green]({x}, {y})[/]")
            elif action_type == "m2":
                x, y = pyautogui.position()
                actions.append(("mouse2", x, y))
                console.print(f"Posição do duplo clique registrada: [bold green]({x}, {y})[/]")
            elif action_type == 'k':
                ...
                if key in pyautogui.KEYBOARD_KEYS:
                    ...
                    console.print("Tecla inválida. Por favor, tente novamente.", style="bold red")
            elif action_type == 't':
...
def save(actions):
    nome_arquivo = Prompt.ask("Digite o nome do software utilizado", default="actions.json")
    ...
    for i in range(n):
        ...
        for action in actions:
            if action[0] == 'mouse':
                ...
                console.print(f"Duplo clique em [bold green]({action[1]}, {action[2]})[/]")
